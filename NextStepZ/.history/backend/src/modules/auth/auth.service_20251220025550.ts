...
export class AuthService {
  ...
  async register(registerDto: RegisterDto): Promise<RegisterResponseDto> {
    ...
    if (existingUser) {
      throw new ConflictException('Email, số điện thoại hoặc username đã được đăng ký');
    }
    ...
    const username =
      ...
      `${registerDto.firstName.toLowerCase()}${registerDto.lastName.toLowerCase()}${Date.now()}`.replace(/\s+/g, '');
  ...
  async sendForgotPasswordOtp(emailOrPhone: string): Promise<{ message: string; success: boolean; email?: string }> {
    ...
    if (!user) {
      ...
      return { message: 'Nếu email/số điện thoại tồn tại, bạn sẽ nhận được mã OTP', success: false };
    }
    ...
    console.log(
      `[OTP-RATE-LIMIT] Email: ${user.email}, lastOtpSentAt: ${lastOtpSentAt}`,
    );
