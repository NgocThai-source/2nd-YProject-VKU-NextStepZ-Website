...
export class ProfileController {
  ...
  async getOrCreatePublicProfile(@CurrentUser() user: any) {
    console.log('[ProfileController] getOrCreatePublicProfile called, user:', user);
    try {
      const result = await this.profileService.getOrCreatePublicProfile(user.userId);
      console.log('[ProfileController] getOrCreatePublicProfile result:', result);
      return result;
    } catch (error) {
      console.error('[ProfileController] Error in getOrCreatePublicProfile:', error);
      throw error;
    }
  }
  ...
  // Get public profile by share token (public endpoint) - MUST BE BEFORE /public/user/:userId
