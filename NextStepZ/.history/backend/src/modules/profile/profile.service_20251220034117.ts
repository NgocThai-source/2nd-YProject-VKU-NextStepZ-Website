...
export class ProfileService {
  ...
  async getOrCreatePublicProfile(userId: string) {
    console.log('[ProfileService] getOrCreatePublicProfile called for userId:', userId);
    ...
    try {
      ...
      if (!publicProfile) {
        console.log('[ProfileService] Public profile not found, checking if profile exists');
        ...
        if (!profile) {
          console.error('[ProfileService] Profile not found for userId:', userId);
          throw new NotFoundException('Profile not found');
        }

        console.log('[ProfileService] Creating new public profile for profileId:', profile.id);
        ...
        console.log('[ProfileService] Public profile created:', publicProfile.id);
      } else {
        console.log('[ProfileService] Public profile already exists:', publicProfile.id);
      }

      return publicProfile;
    } catch (error) {
      console.error('[ProfileService] Error in getOrCreatePublicProfile:', error);
      throw error;
    }
  }
