...
export function ProfileProvider({ children }: { children: React.ReactNode }) {
  ...
  // Re-fetch profile when user ID changes
  useEffect(() => {
    if (!user?.id) return;
      ...
      try {
        ...
        if (res.ok) {
          ...
          setUserProfile((prev) => ({
            ...
            id: data.id,
            userId: data.userId,
            avatar: data.avatar || prev?.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=User',
            name: data.firstName && data.lastName 
              ? `${data.firstName} ${data.lastName}` 
              : data.username || prev?.name || user.username,
            email: data.email,
    ...
    // Set a small delay to ensure page visibility
    const timer = setTimeout(fetchProfileData, 100);
